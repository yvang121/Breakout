    ...
    public BreakoutWall() {
        for (int i = 0; i < brick.getBrick().getWidth()* 10; i += brick.getBrick().getWidth()) {
            for (int j = 0; j < brick.getBrick().getHeight() * 2; j += brick.getBrick().getHeight()) {
                ...
                add(redBrick.getBrick(), i, j + 100);

                Brick orangeBrick = new Brick(Color.ORANGE);
                add(orangeBrick.getBrick(), i, j + orangeBrick.getBrick().getHeight()*2 + 100);

                Brick yellowBrick = new Brick(Color.YELLOW);
                add(yellowBrick.getBrick(), i, j + yellowBrick.getBrick().getHeight()*4 + 100);

                Brick greenBrick = new Brick(Color.GREEN);
                add(greenBrick.getBrick(), i, j + greenBrick.getBrick().getHeight()*6 + 100);

                Brick cyanBrick = new Brick(Color.CYAN);
                add(cyanBrick.getBrick(), i, j + cyanBrick.getBrick().getHeight()*8 + 100);
            }
        }
    }
}
